...
def syl2_ruku():
    ...
    cursor = db.cursor()
    get_K = get_Kline()
    sql1="SELECT date,code from daban_zd WHERE updown >0 and code like'00%%' and date>='%s' and date<='%s'"%('2021-04-23','2021-11-22')
    cursor.execute(sql1)
    end_list = cursor.fetchall()
    for index in end_list:
...
def suanfa1_use2(day,code,get_K):#用于计算买价
    ...
    highest_price, lowest_price = max(high_price), min(low_price)
    syl = (highest_price - float(data_list[1][5])) / float(data_list[1][5])
    return syl,high_price,low_price,data_list,data_list3,buy_price
...
'12/07/2021': '002708',
    ...
    for date,code in data1.items():
        ...
        for index in stock_list:
            ...
            else:syl,high_price,low_price,data_list,data_list3,buy_price=returnend
